...
**Architecture:** Backend adds a `WizardFeatureGraph` domain type, a `FeatureProposalAgent` service, and threads `scopes` + `dependsOn` through `PlanGeneratorAgent` + `TaskService` into real `SpecTask.dependencies`. Frontend replaces `FeaturesForm` with a Rete.js v2 graph editor (`rete-auto-arrange-plugin` for layout, `addPipe` for cycle prevention) and a `useResizable` split-pane side panel. **Graph state lives in the existing `wizard-store`**, consumers subscribe via atomic selectors with `useShallow`. **`applyGraph` uses an incremental diff** against a `Map` held in the editor closure — nodes are added/removed/updated, never full-rebuilt — and listens **only** to `nodedragged` (user drag-end), not `nodetranslated`. Legacy-feature normalization (UUID assignment) happens **exclusively** in the backend's `parseWizardFeatures`.

**Tech Stack:** Kotlin 2.3 / Spring Boot 4 / Koog 0.7.3 / kotlinx.serialization (backend). Next.js 16 / React 19 / TypeScript / Zustand / Rete.js v2 (`rete`, `rete-area-plugin`, `rete-connection-plugin`, `rete-react-plugin`, new: `rete-auto-arrange-plugin`) / Tailwind CSS 4 (frontend). E2E: Playwright.

**Context — second attempt after code rollback `d37596d`:** The first implementation landed in commits `8d64862..e1a7534` and was reverted on 2026-04-18 after a React-19-Strict-Mode crash in the Rete integration produced a browser hang. All code was rolled back (safety branch `backup/pre-rollback-feature22`); docs remained. This plan is the re-implementation basis — all 19 tasks are re-executed, Tasks 13, 15, 16, 17 are redesigned to prevent the crash, and Task 20 (smoke test) is new.
...
- [`docs/superpowers/specs/2026-04-18-feature-22-graph-editor-redesign.md`](../specs/2026-04-18-feature-22-graph-editor-redesign.md) — frontend integration redesign (5 decisions: atomic selectors, 500ms debounce, incremental diff in editor closure, backend-only normalization, smoke-test task)
...
### Task 13: Extend `wizard-store` with graph actions + atomic selectors

**Files:**
- Modify: `frontend/src/lib/stores/wizard-store.ts`

**Design note (from `2026-04-18-feature-22-graph-editor-redesign.md`):** Graph state lives under `data.steps.FEATURES.fields.features` / `.edges` — no separate graph-store. Mutations route through the existing `updateField()` action, which already debounces saves at 500 ms. Consumers must subscribe via **atomic selectors with `useShallow`** — never via a raw `useWizardStore()` subscribe, which re-renders on every store change. Legacy features without `id` are normalized **only** in the backend's `parseWizardFeatures` — this task adds no frontend normalization.

- [ ] **Step 1: Add selector helpers as named exports**

Add these exports to `wizard-store.ts` alongside the `useWizardStore` export so components can import them directly. They are plain functions that take a `WizardState`, suitable for `useWizardStore(useShallow(selectFeatures))`.

```ts
// frontend/src/lib/stores/wizard-store.ts
import type { WizardFeature, WizardFeatureEdge, WizardFeatureGraph } from "@/lib/api";
...
- [ ] **Step 2: Add graph-action type signatures to `WizardState`**

Extend the existing `interface WizardState`:
...
interface WizardState {
  ...
  // graph mutations — all route through updateField() for the existing 500 ms debounce:
  addFeature: (f: Omit<WizardFeature, "id">) => string;               // returns new UUID
...
- [ ] **Step 3: Implement graph actions inside the `create<WizardState>(...)` call**

Add these action implementations next to the existing actions. Import `wouldCreateCycle` at the top:

```ts
import { wouldCreateCycle } from "@/lib/graph/cycleCheck";

// inside create<WizardState>((set, get) => ({ ... }))
...
**Do not add** any `normalizeFeature` helper, `persistNormalizeFeatures`, or `crypto.randomUUID()` fallback for missing IDs. The backend's `parseWizardFeatures` (Task 2) is the single source of truth for legacy migration.
...
git commit -m "feat(frontend): wizard-store graph actions + atomic selectors (Feature 22)"
```
...
**Design note (from `2026-04-18-feature-22-graph-editor-redesign.md`):** The panel receives the active `feature` as a prop — the parent (`FeaturesGraphEditor`) uses an atomic selector (`selectFeatureById(id)` with `useShallow`) to get it out of the store. The panel itself subscribes to the store only via atomic action selectors (`useWizardStore((s) => s.updateFeature)`), never via `useWizardStore()` with destructuring — that pattern subscribes to the whole store and re-renders on every change.
...
export function FeatureSidePanel({ feature, allowedScopes, onDelete }: Props) {
  // Atomic selector — subscribes only to this action, not to the full store.
...
**Design notes (from `2026-04-18-feature-22-graph-editor-redesign.md`):**

1. **No full rebuild in `applyGraph`.** The editor holds a `Map<featureId, RenderedState>` in its closure. Every `applyGraph(features, edges)` call diffs against that map: add new nodes, remove missing ones, update changed titles/scopes/positions in place. Rete's ReactPlugin calls `root.unmount()` synchronously from `editor.removeNode()` — this is fatal during React-19-Strict-Mode render. The diff keeps removes to actual deletions only.
2. **Listen to `nodedragged`, not `nodetranslated`.** `nodedragged` fires only at user drag-end. `nodetranslated` also fires for programmatic `area.translate(...)` calls in `applyGraph`, which would create a feedback loop: translate → event → store write → re-render → applyGraph → translate.
3. **Coalescing queue.** `applyGraph` guards against parallel runs (`applying` flag + one-slot `pending`). A second call while one is in flight overwrites the pending slot (newest wins); concurrent `removeNode`/`addConnection` would otherwise race and throw "cannot find connection".
4. **Atomic selectors in the React component.** `selectFeatures`, `selectEdges`, and each action is subscribed individually via `useWizardStore((s) => ...)` or `useShallow`. Never destructure the store — that subscribes to every change.
5. **Fingerprint-gated `useEffect`.** The sync effect depends on a fingerprint string of features + edges, not on `data` object identity. Other wizard steps mutating the store do not re-trigger `applyGraph`.
  ...
  async function applyGraphOnce(features: WizardFeature[], edges: WizardFeatureEdge[]) {
    ...
    for (const [featureId, reteNodeId] of Array.from(nodeIdByFeatureId.entries())) {
      if (!desiredFeatures.has(featureId)) {
        try { await editor.removeNode(reteNodeId); } catch { /* may already be gone */ }
    ...
    for (const [connId, edgeId] of Array.from(edgeIdByConnectionId.entries())) {
      if (!desiredEdges.has(edgeId)) {
        try { await editor.removeConnection(connId); } catch { /* may already be gone */ }
    ...
    for (const e of edges) {
      ...
      const conn = new ClassicPreset.Connection(fromNode, "out", toNode, "in");
...
export function FeaturesGraphEditor({ projectId }: Props) {
  ...
  const fingerprint = useMemo(() => {
    const f = features
      .map((x) => `${x.id}:${x.title}:${[...x.scopes].sort().join(",")}:${x.position.x}:${x.position.y}`)
      .join("|");
...
- [ ] **Step 4: Smoke-test (manual, with DevTools console open)**

Start backend + frontend (`./start.sh`), create a new project with category `SaaS`, run the wizard to the FEATURES step. Verify (in this order):

- Opening the FEATURES step mounts the editor **without** any red entries in the browser console. No "Cannot update component while rendering", no "Attempted to synchronously unmount a root", no "cannot find connection".
- Clicking `+ Feature` adds a node — the browser stays responsive, the Explorer tree on the left keeps reacting to clicks.
- Clicking a node shows it in the side panel. Renaming it updates the label in the canvas **without** a flurry of re-renders (React DevTools Profiler: one render per keystroke in the panel, not a storm).
- Dragging between ports creates an edge.
- Attempting to create a cycle-forming edge is rejected (no edge drawn, a toast/warning appears).
- `Auto-Layout` tidies nodes.
- `Vorschlagen` calls the backend (requires `OPENAI_API_KEY` to return real content).
- Dragging a node and releasing fires `nodedragged` exactly once (inspect with a temporary `console.log` in the event pipe if needed) — the resulting `moveFeature` call then triggers one auto-save after 500 ms.
- Hard reload (Cmd+R): all features and edges reappear at their previous positions, IDs identical.
...
**Design note (from `2026-04-18-feature-22-graph-editor-redesign.md`):** Same atomic-selector discipline as `FeaturesGraphEditor` — never destructure the store.
...
export function FeaturesFallbackList({ projectId: _ }: Props) {
  // Atomic selectors — each subscribes to its own slice only.
  const features = useWizardStore(useShallow(selectFeatures));
  const edges = useWizardStore(useShallow(selectEdges));
  const category = useWizardStore((s) => s.data?.steps.IDEA?.fields.category as string | undefined);
  const addFeature = useWizardStore((s) => s.addFeature);
...
- Create: `frontend/e2e/features-graph.spec.ts`
- Create: `frontend/e2e/fixtures/seed-project.ts`
- Modify: `frontend/package.json` (add `@playwright/test` dev dep + `test:e2e` script)
- Modify: `frontend/.gitignore` (add `/test-results/`, `/playwright-report/`, `/playwright/.cache/`)

**Design note (from `2026-04-18-feature-22-graph-editor-redesign.md`):** This task adds the first Playwright test to the repo. It seeds the backend project state via REST (not via the wizard UI) so the test does not depend on `OPENAI_API_KEY`. It then drives the browser against a real running dev stack (`./start.sh` must be up).
...
- [ ] **Step 2: Add npm script and gitignore entries**
...
Note: no `webServer` block — the test assumes a running `./start.sh` (both backend and frontend must be up). CI wiring is out of scope for this task.

- [ ] **Step 4: Write the seed fixture `frontend/e2e/fixtures/seed-project.ts`**

This helper creates a project and pre-fills IDEA–MVP via the save (not complete) endpoints — so no LLM calls happen.
...
export async function seedSaaSProjectToFeatures(
  ...
  const createRes = await request.post(`${API}/api/v1/projects`, {
    data: { name: `e2e-features-${Date.now()}` },
  });
  if (!createRes.ok()) throw new Error(`create failed ${createRes.status()}`);
  const project = await createRes.json();
  const projectId = project.id as string;

  // 2. Seed IDEA..MVP via PUT /wizard/{step} (save only, no agent trigger)
  ...
  for (const [step, fields] of steps) {
    ...
    if (!res.ok()) throw new Error(`seed ${step} failed ${res.status()}`);
  }
...
Note on the request shape: confirm the exact `PUT /wizard/{step}` payload by reading `backend/src/main/kotlin/com/agentwork/productspecagent/api/WizardController.kt` before writing this helper — adjust the `data` object to the controller's expected DTO.

- [ ] **Step 5: Write the E2E smoke test `frontend/e2e/features-graph.spec.ts`**

```ts
// frontend/e2e/features-graph.spec.ts
import { test, expect } from "@playwright/test";
import { seedSaaSProjectToFeatures } from "./fixtures/seed-project";

test.describe("Feature 22 — Features graph editor", () => {
  test("mount, add, rename, connect, reload — no console errors, stays responsive", async ({ page, request }) => {
    const { projectId } = await seedSaaSProjectToFeatures(request);

    // Capture console errors from the page (we expect zero React-render-phase errors)
    ...
    // 1. Open the project at the FEATURES step
    await page.goto(`/projects/${projectId}`);
    await page.getByRole("button", { name: /features/i }).click();

    // 2. Wait for editor canvas to mount
    await expect(page.locator('[class*="rete"]').first()).toBeVisible({ timeout: 5_000 });

    // 3. Add first feature — canvas shows one node
    await page.getByRole("button", { name: /\+?\s*feature$/i }).click();
    await expect(page.locator(".rete-node, [data-testid=\"feature-node\"]")).toHaveCount(1, { timeout: 2_000 });

    // 4. Rename via side panel
    const titleInput = page.getByLabel(/titel/i);
    await titleInput.fill("Authentication");
    // Canvas label reflects the new title
    await expect(page.locator("text=Authentication")).toBeVisible({ timeout: 2_000 });

    // 5. Add second feature
    await page.getByRole("button", { name: /\+?\s*feature$/i }).click();
    await expect(page.locator(".rete-node, [data-testid=\"feature-node\"]")).toHaveCount(2, { timeout: 2_000 });

    // 6. Drag a connection from output port of node 1 to input port of node 2
    //    (Rete ports render as .output and .input — exact selector may need
    //    tweaking after Task 14's FeatureNode CSS lands. Use first/last.)
    const outPort = page.locator(".output .socket").first();
    const inPort = page.locator(".input .socket").last();
    const outBox = await outPort.boundingBox();
    const inBox = await inPort.boundingBox();
    if (!outBox || !inBox) throw new Error("ports not found — update selector");
    ...
    // 7. Connection rendered — there's now exactly one connection path in the editor
    await expect(page.locator(".connection, path.connection, [data-type=\"connection\"]")).toHaveCount(1, { timeout: 2_000 });

    // 8. Wait for debounced save (500ms + network)
    await page.waitForTimeout(1200);

    // 9. Hard reload — everything persists
    await page.reload();
    await page.getByRole("button", { name: /features/i }).click();
    await expect(page.locator(".rete-node, [data-testid=\"feature-node\"]")).toHaveCount(2, { timeout: 5_000 });
    await expect(page.locator("text=Authentication")).toBeVisible();

    // 10. Explorer tree stays responsive throughout — click into it to confirm
    //     (the original bug hung the entire UI). We just click its root node.
    const explorerRoot = page.getByRole("tree").first();
    if (await explorerRoot.isVisible()) {
      await explorerRoot.click();
    }

    // 11. No React render-phase errors on the page
    expect(consoleErrors, `unexpected console errors:\n${consoleErrors.join("\n")}`).toEqual([]);
  });
});
```

If the selectors (`.rete-node`, `.output .socket`, `.input .socket`) don't match once Task 14's `FeatureNodeComponent` is in place, add `data-testid` hooks to `FeatureNode.tsx` (`data-testid="feature-node"`, `data-testid="port-output"`, `data-testid="port-input"`) and update the selectors — do the DOM adjustments in this task, not by modifying Task 14 retroactively.

- [ ] **Step 6: Run the test against a live dev stack**
...
Expected: test passes. No console errors collected, 2 nodes visible after reload, connection persisted.

- [ ] **Step 7: Commit**

```bash
git add frontend/package.json frontend/package-lock.json frontend/.gitignore frontend/playwright.config.ts frontend/e2e
git commit -m "test(frontend): Playwright smoke test for features graph (Feature 22)"
```
...
**Spec coverage** — every requirement in `docs/features/22-features-graph-wizard.md` maps to a task: domain types (T1), backward-compat parse (T2), scope-aware plan gen (T3), dependency mapping (T4), graph block in agent context (T5), validator clarifications (T6), proposal agent (T7), proposal endpoint (T8), scaffold real deps + scope sections (T9), plugin dep (T10), frontend types (T11), cycle util (T12), wizard-store + atomic selectors (T13), Rete node (T14), side panel with atomic action selectors (T15), main editor with incremental `applyGraph` + `nodedragged`-only + coalescing queue (T16), mobile fallback with atomic selectors (T17), wire-up (T18), regression (T19), Playwright smoke test (T20). Redesign decisions from `2026-04-18-feature-22-graph-editor-redesign.md` are covered by T13 (no frontend normalization, atomic selectors, graph actions via `updateField` for the 500 ms debounce), T15–T17 (atomic-selector discipline), T16 (incremental diff, `nodedragged`-only, coalescing queue), T20 (smoke-test acceptance). Acceptance-criteria-level gaps: none.

**Placeholder scan** — Two intentional `TODO` markers remain inside **test code comments** in Task 3 and Task 4 ("reuse helper from existing ...Test"). They are explicitly called out as implementer notes to copy existing test fixtures, not plan-level placeholders. One `TODO_read_category` marker in Task 5's `buildProposalContext` pseudocode is flagged with an implementer note explaining where to read the real value. Task 20 contains one implementer-note about verifying the `PUT /wizard/{step}` payload shape against the real controller DTO — this is a call-out, not a gap. No unresolved "fill in later" or "add error handling" steps remain.

**Type consistency** — `WizardFeatureInput` signature is identical across T2, T3, T4, T6. `WizardFeature` front-end TS type mirrors Kotlin domain. `FeatureScope` enum values (`FRONTEND`/`BACKEND`) match between Kotlin and TypeScript. `SCOPE_FIELDS_BY_SCOPE` uses `CORE` key only in TypeScript (not an enum — scope is empty set in Kotlin; that's the correct asymmetry). `scopeFields` is `Map<String, String>` everywhere. Frontend `WizardFeature.scopes` is `FeatureScope[]` (array) across T11, T13, T14, T15, T16, T17 — the editor's `scopesKey` helper in T16 sorts-and-joins the array for change detection. Selector names (`selectFeatures`, `selectEdges`, `selectFeatureById`) match between their definition in T13 and all consumers (T16, T17).

---

## Execution Handoff

Plan complete and saved to `docs/superpowers/plans/2026-04-18-feature-22-features-graph-wizard.md`. Two execution options:
