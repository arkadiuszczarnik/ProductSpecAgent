...
Im `FeatureEditDialog` lassen sich pro Feature Akzeptanzkriterien als geordnete Liste pflegen — jeder Eintrag ist eine einzige Text-Zeile (z. B. *„Aufruf von `/` ohne Cookie → automatisch redirect zu `/login`."*). Ein „AC vorschlagen"-Button startet einen LLM-Agenten, der 3–6 stakeholder-orientierte Kriterien generiert; Vorschläge werden an die bestehende Liste angehängt. Der `ScaffoldContextBuilder` nimmt die neuen Wizard-AC als primäre Quelle, mit Fallback auf Story-Subtasks für bestehende Projekte.

## Architektur

Siehe Design-Spec für das vollständige Bild. Kurzfassung:

- **Backend Domain (`WizardFeatureGraph.kt`):** neuer `AcceptanceCriterion(id, text)`-Datentyp; neues Feld `acceptanceCriteria: List<AcceptanceCriterion> = emptyList()` in `WizardFeature`.
...
- Ein Single-Line-`Input` pro Eintrag enthält den Kriterien-Text (z. B. *„Aufruf von `/` ohne Cookie → automatisch redirect zu `/login`."*).
- ✕-Button entfernt einen Eintrag sofort, ohne Confirm.
- ↑/↓-Buttons verschieben um eine Position; bei Index-Grenzen disabled.
- Enter im Input fügt einen leeren Eintrag direkt **darunter** ein und fokussiert ihn.
...
- Beim Speichern werden Einträge mit leerem Text (nach Trim) still gefiltert.
...
Mustache-Template (`feature.md.mustache`) rendert nur noch den Text:
```mustache
- [ ] {{title}}
```
(`TaskContext.title` trägt im Wizard-AC-Pfad den AC-Text; im Subtask-Fallback weiterhin den Subtask-Titel.)
...
2. „+ Akzeptanzkriterium hinzufügen" hängt einen leeren Eintrag an und fokussiert das Text-Input.
3. Pro Eintrag steht ein einzeiliges `Input`-Feld für den Kriterien-Text zur Verfügung.
4. ✕-Button entfernt einen Eintrag sofort, ohne Confirm.
5. ↑/↓-Buttons verschieben um eine Position; bei Index-Grenzen disabled.
6. Enter im Input fügt einen neuen leeren Eintrag direkt darunter ein und fokussiert ihn.
7. Eingaben mutieren `draft`; Speichern erst beim Klick auf „Speichern".
8. Beim Speichern werden Einträge mit leerem Text (Trim) still gefiltert.
...
**Backend / API:**
11. `POST /api/v1/projects/{projectId}/features/{featureId}/acceptance-criteria/propose` liefert `200 OK` mit `List<AcceptanceCriterion>` (3–6 Einträge mit UUID und nicht-leerem Text).
...
18. `feature.md` zeigt AC im Format `- [ ] {text}` (eine Zeile pro Kriterium).
